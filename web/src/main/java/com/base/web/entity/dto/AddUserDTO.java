...
public class AddUserDTO {
    ...
    @Pattern(regexp = PatternUtil.PASSWORD_REG, message = "用户名格式不正确")
    ...
    @Pattern(regexp = PatternUtil.PASSWORD_REG, message = "密码首个必须是字母，有数字字母下划线组成,长度为6-20位")
    private String password;

    @ApiModelProperty(value = "邮箱", hidden = true)
//    @NotBlank(message = "邮箱不能为空")
//    @Pattern(regexp = PatternUtil.EMAIL_REG, message = "邮箱格式不正确")
...
//    @Pattern(regexp = PatternUtil.TELEPHONE_REG, message = "手机号码格式不正确")
    ...
    @ApiModelProperty(value = "用户角色(数组里面是角色id),如果不修改,则默认普通用户", hidden = true)
